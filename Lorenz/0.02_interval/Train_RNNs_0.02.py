...
length = 200
total = 5000
xyz1 = np.loadtxt('0.0_0.5_0.0_xyz')[::2,:].reshape(total//length,length,3)
input1 = xyz1[:,:-1,:]
output1 = xyz1[:,1:,:]
# print(input1.shape,output1.shape)

xyz2 = np.loadtxt('1.0_1.0_1.0_xyz')[::2,:].reshape(total//length,length,3)
input2 = xyz2[:,:-1,:]
output2 = xyz2[:,1:,:]

xyz3 = np.loadtxt('2.0_2.0_2.0_xyz')[::2,:].reshape(total//length,length,3)
input3 = xyz3[:,:-1,:]
output3 = xyz3[:,1:,:]
...
xyzV = np.loadtxt('0.0_1.5_1.5_xyz')[::2,:].reshape(total//length,length,3)
inputV = xyzV[:,:-1,:]
outputV = xyzV[:,1:,:]
...
RNN_model = torch.load("model_LSTM_0.02")
...
def train(epochs):
    ...
    for epoch in range(epochs):
        ...
        torch.save(RNN_model, "model1")

    return train_loss

# _ = train(1601)

# torch.save(RNN_model, "model_LSTM_0.02")
...
test_input = torch.Tensor(np.loadtxt('0.2_0.2_0.3_xyz'))[::2,:]
test_result = np.loadtxt('0.2_0.2_0.3_xyz')[::2,:]
...
# test_result = np.loadtxt('0.2_0.2_0.3_xyz')[::2,:]
test_result = np.loadtxt('0.2_0.2_0.3_xyz')[::2,:][test_length:,:]
