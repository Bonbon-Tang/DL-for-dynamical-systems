...
forceV = np.loadtxt('0.5_0.5_input_2%').reshape(300,1)
stateV = np.loadtxt('0.5_0.5_state_2%').reshape(301,2)
...
model = torch.load("model_1step_2%")
...
def train(epochs):
    ...
    for epoch in range(epochs):
            ...
            if loss < min_loss:
                min_loss = loss
                torch.save(model, "model_1step_2%")
            if epoch % 200 == 0:
