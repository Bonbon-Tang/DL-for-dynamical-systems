...
interval = 5
total = 2000
xyz1 = np.loadtxt('0.0_0.5_0.0_xyz')[::interval,:]
input1 = xyz1[:total-step,:]
for i in range(1,step):
    input_cur = xyz1[i:total-step+i]
    input1 = np.concatenate((input1,input_cur),axis=1)
output1 = xyz1[step:,:]
# print(input1.shape,output1.shape)

xyz2 = np.loadtxt('1.0_1.0_1.0_xyz')[::interval,:]
input2 = xyz2[:total-step,:]
for i in range(1,step):
    input_cur = xyz2[i:total-step+i]
    input2 = np.concatenate((input2,input_cur),axis=1)
output2 = xyz2[step:,:]

xyz3 = np.loadtxt('2.0_2.0_2.0_xyz')[::interval,:]
input3 = xyz3[:total-step,:]
for i in range(1,step):
    input_cur = xyz3[i:total-step+i]
...
xyzV = np.loadtxt('0.0_1.5_1.5_xyz')[::interval,:]
inputV = xyzV[:total-step,:]
for i in range(1,step):
    input_cur = xyzV[i:total-step+i]
...
# _,_ = train(8001)
# torch.save(model, "model_FNN_0.05")
...
test_input = torch.Tensor(np.loadtxt('0.2_0.2_0.3_xyz'))[::interval,:]
test_result = np.loadtxt('0.2_0.2_0.3_xyz')[::interval,:]
...
print(np.mean(np.square(predict_result-test_result))* length / (length - 1))
