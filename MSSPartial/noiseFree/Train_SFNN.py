...
stateV = np.loadtxt('0.0_1.0_state')[:,0].reshape(201,1)
...
model = nn.Sequential(nn.Linear(1*step,20),
                      nn.ReLU(),
                      nn.Linear(20,1),
                     )
...
# torch.save(model, "model_FNN2_20")
...
test_input = torch.Tensor(np.loadtxt(testName+'state')[:step,0].reshape(step,1))
