...
step = 1
force1 = np.loadtxt('0.0_0.0_input_2%').reshape(300,1)
state1 = np.loadtxt('0.0_0.0_state_2%').reshape(301,2)
input1 = np.concatenate((state1[:-step,:],force1[:300-step+1,:]),axis = 1)
# print(input1.shape)

force2 = np.loadtxt('0.0_-1.0_input_2%').reshape(300,1)
state2 = np.loadtxt('0.0_-1.0_state_2%').reshape(301,2)
input2 = np.concatenate((state2[:-step,:],force2[:300-step+1,:]),axis = 1)

force3 = np.loadtxt('1.0_1.0_input_2%').reshape(300,1)
state3 = np.loadtxt('1.0_1.0_state_2%').reshape(301,2)
input3 = np.concatenate((state3[:-step,:],force3[:300-step+1,:]),axis = 1)

force4 = np.loadtxt('2.0_1.5_input_2%').reshape(300,1)
state4 = np.loadtxt('2.0_1.5_state_2%').reshape(301,2)
...
train_output = (np.concatenate((state1[step:,:],state2[step:,:],state3[step:,:],state4[step:,:]), axis=0)
                - np.concatenate((state1[:-step,:],state2[:-step,:],state3[:-step,:],state4[:-step,:]), axis=0))
train_input = torch.Tensor(train_input)
train_output = torch.Tensor(train_output)
# print(train_input.shape,train_output.shape)

forceV = np.loadtxt('0.5_0.5_input').reshape(300,1)
stateV = np.loadtxt('0.5_0.5_state').reshape(301,2)
inputV = np.concatenate((stateV[:-step,:],forceV[:300-step+1,:]),axis = 1)
outputV = stateV[step:,] - stateV[:-step,]
...
# train_loss,val_loss = train(1001)
torch.save(model, "model_1step_2%_dt")

test_initial = torch.Tensor(np.loadtxt('2.0_1.0_state')[0,:].reshape(1,2))
test_force = torch.Tensor(np.loadtxt('2.0_1.0_input').reshape(300,1))
test_result = np.loadtxt('2.0_1.0_state')[1:,:].reshape(300,2)
predict_result = np.zeros(shape=(300,2))

pre_result = test_initial
one_result = model(torch.cat((test_initial,test_force[:1,:]),1))
one_result = one_result[:, :2] +  pre_result[:,:2]
predict_result[0,:] = one_result.detach().numpy()
pre_result = one_result

for i in range(1,300):
    one_input = torch.cat((pre_result, test_force[i,:].reshape(1,1)),1)
