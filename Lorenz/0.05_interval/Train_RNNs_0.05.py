...
length = 200
total = 2000
xyz1 = np.loadtxt('0.0_0.5_0.0_xyz')[::5,:].reshape(total//length,length,3)
input1 = xyz1[:,:-1,:]
output1 = xyz1[:,1:,:]
# print(input1.shape,output1.shape)

xyz2 = np.loadtxt('1.0_1.0_1.0_xyz')[::5,:].reshape(total//length,length,3)
input2 = xyz2[:,:-1,:]
output2 = xyz2[:,1:,:]

xyz3 = np.loadtxt('2.0_2.0_2.0_xyz')[::5,:].reshape(total//length,length,3)
input3 = xyz3[:,:-1,:]
output3 = xyz3[:,1:,:]
...
xyzV = np.loadtxt('0.0_1.5_1.5_xyz')[::5,:].reshape(total//length,length,3)
inputV = xyzV[:,:-1,:]
outputV = xyzV[:,1:,:]
...
RNN_model = torch.load("model1")
criterion = nn.MSELoss()
optimizer = torch.optim.Adam(RNN_model.parameters(),lr=0.02)
...
torch.save(RNN_model, "model_LSTM_0.05")
...
test_input = torch.Tensor(np.loadtxt('0.2_0.2_0.3_xyz'))[::5,:]
test_result = np.loadtxt('0.2_0.2_0.3_xyz')[::5,:]
...
test_result = np.loadtxt('0.2_0.2_0.3_xyz')[::5,:]
# test_result = np.loadtxt('0.2_0.2_0.3_xyz')[::5,:][test_length:,:]
...
print(np.mean(np.square(predict_result-test_result))* test_length / (test_length - 1))
