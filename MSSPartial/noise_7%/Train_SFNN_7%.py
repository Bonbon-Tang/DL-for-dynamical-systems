...
state3 = np.loadtxt('-2.0_1.0_state_7%')[:,0].reshape(201,1)
input3 = state3[:201 -step, :]
for i in range (1,step):
    input_cur = state3[i:201-step+i,:]
    input3 =  np.concatenate((input3,input_cur),axis= 1)

state4 = np.loadtxt('-1.0_-1.0_state_7%')[:,0].reshape(201,1)
...
stateV = np.loadtxt('0.0_1.0_state_7%')[:,0].reshape(201,1)
...
# torch.save(model, "model_FNN3_32_7%")

testNames = ['0.5_0.5_','-0.5_0.5_','1.0_0.0_']
result = []
for testName in testNames:

    test_state = torch.Tensor(np.loadtxt(testName+'state_7%').reshape(201,2))
    test_input = torch.Tensor(np.loadtxt(testName+'state_7%')[:step,0].reshape(step,1))
    test_result = np.loadtxt(testName+'state_7%')[step:,0].reshape(201-step,1)
