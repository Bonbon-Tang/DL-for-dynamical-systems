...
forceV = np.loadtxt('0.5_0.5_input').reshape(300,1,1)
stateV = np.loadtxt('0.5_0.5_state').reshape(301,1,2)
...
Transformer  = TransformerModel(intoken=3, outtoken=3,final=2, hidden=24, nlayers=3, dropout=0.0)
...
def evaluate (length):
    ...
    for i in range (length):
        Transformer.eval()
        if i > 1:
            output = Transformer(val_src, decoder_input[-1:,:,:])
        else:
