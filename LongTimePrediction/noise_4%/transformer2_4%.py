...
class TransformerModel(nn.Module):
    def __init__(self, intoken, outtoken,final, hidden, nlayers=2, dropout=0.1):
    ...
    def forward(self, src):
        ...
        # output = self.transformer(src,mask = None)
        output = self.fc_out(output)
        return output

Transformer = TransformerModel(intoken=3, outtoken=3, final=2, hidden=24, nlayers=3, dropout=0.1)
...
# trainLoss = train(601)
# torch.save(Transformer, "Transformer_2%_withoutPE")


forceT = np.loadtxt('0.5_0.5_input').reshape(300,1,1)
stateT = np.loadtxt('0.5_0.5_state').reshape(301,1,2)
