...
stateV = np.loadtxt('0.0_1.0_state_2%')[:,0].reshape(201,1)
...
for m in model.modules():
   if isinstance(m,(nn.Linear)):
    ...
    # model = torch.load("model_FNN4_32_2%")
...
# torch.save(model, "model_FNN5_32_2%")

testNames = ['0.5_0.5_','-0.5_0.5_','1.0_0.0_']
result = []
for testName in testNames:

    test_state = torch.Tensor(np.loadtxt(testName+'state_2%').reshape(201,2))
    test_input = torch.Tensor(np.loadtxt(testName+'state_2%')[:step,0].reshape(step,1))
    test_result = np.loadtxt(testName+'state_2%')[step:,0].reshape(201-step,1)
