...
step = 1
force1 = np.loadtxt('0.0_0.0_input').reshape(300,1)
state1 = np.loadtxt('0.0_0.0_state').reshape(301,2)
input1 = np.concatenate((state1[:-step,:],force1[:300-step+1,:]),axis = 1)
# print(input1.shape)

force2 = np.loadtxt('0.0_-1.0_input').reshape(300,1)
state2 = np.loadtxt('0.0_-1.0_state').reshape(301,2)
input2 = np.concatenate((state2[:-step,:],force2[:300-step+1,:]),axis = 1)

force3 = np.loadtxt('1.0_1.0_input').reshape(300,1)
state3 = np.loadtxt('1.0_1.0_state').reshape(301,2)
input3 = np.concatenate((state3[:-step,:],force3[:300-step+1,:]),axis = 1)

force4 = np.loadtxt('2.0_1.5_input').reshape(300,1)
state4 = np.loadtxt('2.0_1.5_state').reshape(301,2)
...
model = torch.load("model_1step_dt")
...
# torch.save(model, "model_1step_dt")

test_initial = torch.Tensor(np.loadtxt('0.0_1.5_state')[0,:].reshape(1,2))
test_force = torch.Tensor(np.loadtxt('0.0_1.5_input').reshape(300,1))
test_result = np.loadtxt('0.0_1.5_state')[1:,:].reshape(300,2)
