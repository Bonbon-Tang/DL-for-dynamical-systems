...
step = 14
hiddenSize = 32
state1 = np.loadtxt('1.0_2.0_state_20%')[:,0].reshape(201,1)
input1 = state1[:201 -step, :]
for i in range (1,step):
    input_cur = state1[i:201-step+i,:]
    input1 =  np.concatenate((input1,input_cur),axis= 1)

state2 = np.loadtxt('2.0_0.5_state_20%')[:,0].reshape(201,1)
...
state3 = np.loadtxt('-2.0_1.0_state_20%')[:,0].reshape(201,1)
input3 = state3[:201 -step, :]
for i in range (1,step):
    input_cur = state3[i:201-step+i,:]
    input3 =  np.concatenate((input3,input_cur),axis= 1)

state4 = np.loadtxt('-1.0_-1.0_state_20%')[:,0].reshape(201,1)
...
stateV = np.loadtxt('0.0_1.0_state_20%')[:,0].reshape(201,1)
...
train_loss,val_loss = train(6001)
# torch.save(model, "model_FNN12_32_20%")

testNames = ['0.5_0.5_','-0.5_0.5_','1.0_0.0_']
result = []
for testName in testNames:

    test_state = torch.Tensor(np.loadtxt(testName+'state_20%').reshape(201,2))
    test_input = torch.Tensor(np.loadtxt(testName+'state_20%')[:step,0].reshape(step,1))
