...
#
forceV = np.loadtxt('1.0_0.0_input').reshape(1, 300, 1)
stateV = np.loadtxt('1.0_0.0_state').reshape(1, 301, 2)
...
# torch.save(GRU_model, "GRU_32_model")
torch.save(GRU_model, "LSTM_32_model")
...
test_force = np.loadtxt(testName+'input').reshape(1, 300, 1)
test_state = np.loadtxt(testName+'state').reshape(1, 301, 2)
test_output = test_state[:,100:150,:]
